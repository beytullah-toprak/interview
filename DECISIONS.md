...
src/
  ...
  Helpers/
    ...
    MoneyHelper.php           # tutarları aktif dile göre biçimlendirir
  ...
  classes/
    ...
    Order.php                 # /order-token + /order uçları, sipariş akışı
  languages/                  # tr.php / en.php (key setleri birebir eşleşiyor)
  templates/                  # index.html / home.html / 404.html
assets/
  css/app.css
  js/app.js
tests/
  ...
  Helpers/MoneyHelperTest.php
```

Katman ayrımının mantığı: `TurkpinApiClient` API'ye "nasıl" konuşulacağını bilir
(curl, XML), Service'ler "ne" isteneceğini bilir (iş kuralları), `Home`/`Order`
isteği alıp servise devreder ve cevabı döner, `Main.php` ise yalnızca "hangi
URL hangi sınıfa gider" sorusunu cevaplar.
...
şişiriyordu. Sipariş mantığı `Order.php`'ye çıkarılınca `Main.php` 149
satırdan 80 satıra indi ve routing ile iş mantığı ayrıştı. (`Home.php` ile
aynı isimlendirme desenine uysun diye sınıf `OrderController` değil `Order`
olarak adlandırıldı.)
...
- Ana sayfada dropdown'da gösteriliyor, başlangıçta hiçbir oyun seçili değil
  ("Oyun Seçiniz / Select Game" varsayılan seçenek).
...
- Oyun değişince liste otomatik güncelleniyor (sayfa yenilemesiyle — bkz.
  "Bilinen Eksikler"); yenileme sırasında select disable edilip kullanıcıya
  geri bildirim veriliyor.
- Min./Max. sipariş ve "Ön Sipariş" bilgisi de tabloda gösteriliyor.
- Adet değiştikçe satırın "Tutar" hücresi `adet × birim fiyat` ile anında
  güncelleniyor; kullanıcı "Satın Al"a basmadan ne ödeyeceğini görür.
- Fiyatlar `MoneyHelper`/`formatAmount()` ile aktif dile göre biçimleniyor
  (tr: `1.234,50 ₺`, en: `1,234.50 ₺`); test API'sindeki `0.001` gibi çok
  küçük birim fiyatlar 2-4 basamak arası gösterilerek "0,00" gibi yanıltıcı
  bir görünüme düşmüyor.
- Stokta olmayan ürünlerde adet alanı `0` gösterilir ve devre dışı bırakılır,
  tutar da `0` çıkar — daha önce adet `1` görünüp yanlış bir tutar basıyordu.
...
- **Stokta olmayan ürün sepete eklenemez:** iki katmanlı korunuyor —
  görsel olarak "Stokta Yok" rozeti + adet/buton disable (kullanıcı
  tıklayamaz bile), asıl güvenlik olarak da sunucu tarafında `OrderValidator`
  kullanıcının gönderdiği hiçbir değere güvenmeden ürünü API'den tekrar
  çekip gerçek stok değerine bakıyor. Devtools'tan `disabled` silinip yine de
  istek atılsa dahi sunucu reddediyor.
...
### 4. Sonuç Gösterimi
- Sipariş sonucu SweetAlert2 modal ile gösteriliyor. Tarayıcının yerleşik
  `alert()`'i yerine bilinçli olarak tercih edildi: stilize edilemiyor,
  engelleyici, mobilde çirkin ve başarı/hata ayrımı yapamıyor.
- Başarılı/başarısız durumlar farklı ikon ve stillerle sunuluyor.
- Başarılı siparişte sipariş no ve biçimlendirilmiş tutar gösteriliyor.
...
- Modal mesajları `window.LANG` (Main.php'de $lang dizisinden `json_encode`
  ile üretilir) üzerinden TR/EN dil desteğine tam uyumlu; onay butonunun
  metni bile dile göre değişiyor ("Tamam" / "OK").
...
5. **Kullanılmayan bağımlılık** — `composer.json`'daki `aura/router` hiç
   kullanılmıyordu (proje zaten Bramus Router kullanıyor, kod tabanında
   `Aura\Router`'a tek bir referans bile yoktu); kaldırıldı.
...
9. **`?lang` doğrulanmıyordu (LFI + kalıcı çökme)** — `$_GET['lang']` hiçbir
   ...
   giriyordu. İki ayrı sorun vardı:
   - **Path traversal / LFI:** `?lang=../../../../etc/passwd` gibi bir
     istekle sunucudaki keyfi dosyalar `require` edilebilirdi.
   - **Session zehirlenmesi:** geçersiz değer `require`'dan **önce**
     session'a yazılıyordu; tek bir `?lang=zz` isteği session'ı bozup
     sonraki tüm istekleri (`/` dahil) fatal error'a düşürüyordu —
     kullanıcı cookie'sini silene kadar siteye giremiyordu.

   Çözüm bir whitelist: `SUPPORTED_LANGS = ['tr', 'en']`; hem GET hem
   session değeri bu listeden geçiyor, dışındaki her şey sessizce `tr`'ye
   düşüyor.
...
12. **Parse edilip hiç gösterilmeyen veriler** — `ProductService`
    `min_order`/`max_order`/`pre_order` alanlarını okuyordu ve dil
    dosyalarında karşılıkları hazırdı, ama hiçbiri ekranda gösterilmiyordu.
    Tabloya sütun ve "Ön Sipariş" rozeti olarak eklendi.
...
14. **Mobilde "Satın Al" butonu ekran dışında kalıyordu** — sütun sayısı
    artınca tablo 375px ekranda yatay kaydırmaya düşüyor ve sayfanın var
    oluş sebebi olan buton görünmez oluyordu. README hem "mevcut tablo
    yapısını koruyun" hem "responsive tasarımı bozmayın" dediği için tabloyu
    kart listesine *çevirmek* yerine, HTML tablo yapısı aynı bırakılıp 768px
    altında satırlar CSS ile karta dönüştürüldü (sütun başlıkları her
    hücrenin `data-label` değerinden `::before` ile basılıyor). Markup hâlâ
    tablo, yalnızca sunum değişiyor.
15. **Sipariş token'ları session'da birikiyordu** — token'lar `[$token => true]`
    ...
    30 dakikalık TTL uygulanıyor, süresi geçenler temizleniyor ve aktif
    token sayısı 20 ile sınırlanıyor.
16. **Sonuç bulunamayan sayfalar bomboş dönüyordu** — router hiçbir route
    eşleşmeyince şablona bir şey atanmıyor, kullanıcı boş bir sayfayla
    karşılaşıyordu. `set404()` ile gerçek 404 durum kodu ve TR/EN 404
    sayfası eklendi.
17. **Rozet/buton renkleri okunabilirlik ve marka açısından zayıftı** —
    Bootstrap'in `text-bg-info` rozeti açık mavi zemine siyah yazı
    basıyordu, `text-bg-danger` ise markanın kırmızısıyla çakışıyordu.
    Kendi yumuşak zeminli rozetlerimizle değiştirildi ("Stokta Yok" nötr
    gri — bir hata değil sadece bir durum; "Ön Sipariş" markanın yumuşak
    kırmızı tonu). Dolu kırmızı "Satın Al" butonu da tabloda tekrar edince
    uyarı/silme gibi okunuyordu; koyu zemine, kırmızının yalnızca hover'da
    vurgu olarak devreye girdiği bir stile çevrildi.

---

## Ek Değer Sağlayan Çalışmalar

- **Docker:** `Dockerfile` (PHP 8.2 + Apache, `mod_rewrite`/`headers` aktif)
  ve `docker-compose.yml` (`./:/var/www/html` bind-mount — kod değişince
  container'ı yeniden build etmeye gerek yok) ile tek komutla çalışan ortam.
...
- **Asset versiyonlama:** `AssetHelper`, dosya değişiklik zamanına (`filemtime`)
  göre `?v=...` ekleyerek tarayıcı cache sorununu otomatik çözüyor —
  `app.js`/`app.css` güncellenip deploy edildiğinde kullanıcı elle
  hard-refresh yapmadan yeni dosyayı alır; her deploy'da elle versiyon
  numarası takip etmeye gerek kalmıyor.
...
- **Unit test:** `OrderValidator` (min/max/stok kuralları, `max_order=0`
  ...
  geçersiz XML), `GameService`/`ProductService`/`OrderService` (XML→array
  eşlemesi, `TurkpinApiClient::request()` mock'lanarak) ve `MoneyHelper`
  (TR/EN ayırıcılar, değişken basamak sayısı) için PHPUnit testleri
  eklendi — bkz. `tests/`. Çalıştırmak için: `vendor/bin/phpunit`.
...
- **phpunit/phpunit (dev-only):** `OrderValidator`, `TurkpinApiClient`'ın
  XML parse mantığı ve `MoneyHelper`'ı gerçek ağ isteği atmadan test
  edebilmek için. Runtime'a hiçbir etkisi yok (`require-dev`).
- Başka yeni Composer paketi eklenmedi. HTTP için PHP'nin yerleşik cURL'ü,
  XML için SimpleXML, para biçimlendirme için `number_format` kullanıldı
  (`intl`/`NumberFormatter` bilerek tercih edilmedi — bu eklenti Docker
  imajında yüklü değil, ek eklenti kurulumu gerektirmemek için elle
  yazıldı) — gereksiz bağımlılık eklememek için.
...
- Oyun listesi (`epinOyunListesi`) yalnızca `id` ve `name` dönüyor; ayrı bir
  açıklama/kategori alanı yok.
...
- `Order` sınıfının token/TTL mantığı `$_SESSION`'a doğrudan bağlı olduğu
